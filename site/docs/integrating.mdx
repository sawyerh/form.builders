...
What lies behind a user-friendly form is sometimes a scary series of pipes duct taped together to get the form's response data to its final destination so that it can be processed. This is sometimes a necessary evil in order to reduce risk and deliver value in incremental chunks, rather than taking years to deliver anything.

This can take the form of first understanding existing business processes and finding creative ways to find a wedge. Are staff manually entering data into a spreadsheet? Maybe you can create a process that outputs a CSV, which can then be imported into the spreadsheet. Are staff asking customers to email their forms? Your site could send the form as an email.
